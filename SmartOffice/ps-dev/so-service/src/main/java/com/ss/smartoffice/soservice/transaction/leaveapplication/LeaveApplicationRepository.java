...
@Query("select i from com.ss.smartoffice.soservice.transaction.leaveapplication.LeaveApplication i "
		+ "where ifnull(LOWER(i.employeeId),'') LIKE LOWER(CONCAT('%',ifnull(:employeeId,''), '%')) AND "
		+ "i.startDt >= :startDate AND i.endDt <= :endDate")
Iterable<LeaveApplication> fetchByAdvance(
@Param("employeeId")String employeeId,@Param("startDate")LocalDate startDate,@Param("endDate")LocalDate endDate);
