...
public interface PartnerRepository extends CrudRepository<Partner, Integer>{
	...
	@Query("select x from com.ss.smartoffice.shared.model.partner.Partner x where x.isClient=:isClient order by x.id DESC ")
	 ...
	 @Query("select i from com.ss.smartoffice.shared.model.partner.Partner i where "
				+ "ifnull(LOWER(i.clientCode),'') LIKE LOWER(CONCAT('%',ifnull(:clientCode,''), '%'))  AND ifnull(LOWER(i.companyName),'') LIKE LOWER(CONCAT('%',ifnull(:companyName,''), '%')) AND ifnull(LOWER(i.countryName),'') LIKE LOWER(CONCAT('%',ifnull(:countryName,''), '%')) "
