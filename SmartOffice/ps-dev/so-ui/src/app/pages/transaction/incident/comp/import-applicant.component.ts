...
import { WebSiteApplicant } from './../../../../../../../so-recruitment/src/app/pages/transaction/vacancy-opening/vo/web-site-applicant';
...
export class ImportApplicantComponent implements OnInit {
    ...
    ngOnInit() {  
        ...
        this.displayWebApplications = [new WebApplicant];
  ...
  masterSelected:boolean;
    checkUncheckAll() {
        ...
        for (var i = 0; i < this.displayWebApplications.length;i++){
            this.displayWebApplications[i].select=this.masterSelected; 
        }
     for(let webApplicant of this.displayWebApplications){
        if(webApplicant.imported!='Yes'){
            this.masterSelected==true;
            this.addIncidentApplicant(webApplicant);
        }
    ...
    selectimport(type){
      this.isImported=type;
      this.displayWebApplications = [];
      this.webApplications.forEach(element => {
          if(type=='Yes' && element.imported=='Yes'){
              this.displayWebApplications.push(element)
          }else if(type=='No' && element.imported!='Yes'){
              this.displayWebApplications.push(element)
          }else if(type=='All'){
              this.displayWebApplications.push(element)
          }
