...
import { BankInfo } from '../../bank-info/vo/bank-info';
...
export class EmployeeDetailComponent implements OnInit {
	...
	ngOnInit() {
		...
		this._service.getAcadmicType().subscribe(type=>{
			this.eduType=type;
		});
		this._service.getAcadmicName().subscribe(name=>{
			this.eduName=name;
		})
		...
		.subscribe(params=>{
			...
			if(!this.addScreen){
				...
				this._service.getDocument(x.docId).subscribe(x=>{
					// this.createImageFromBlob(x);
				})
					// this.docLocation="/opt/so/data/docs/upload/APPLICANT-PHOTO/"+this.docName;
	...
	get EduName(){
		return this.eduName.filter(x=>x.abbreviation=='SCHOOL');
	}
	...
	createFormControls(){
		...
		this.lename = new FormControl('', [Validators.required,Validators.pattern('[a-zA-Z0-9][a-zA-Z0-9 ]+')]);
	...
	legalPending(){
		if(this.employee.pfNo==null||this.employee.esiNo==null||this.employee.uanNo==null){
			this.employee.legalPending='Y';
			this.msg = { type: 'danger', text: "official Info PF ,ESI,UAN number are missing will proceed to save and will show Legal Penging Flag in List" }
		}else{
			this.employee.legalPending='N';
		}
	
	}

	projectSelected($event){
		this.employee.n1EmpId = $event.id;
		this.employee.managerName= $event.empName;
	}
	projectSelected1($event){
		this.employee.n2EmpId = $event.id;
		this.employee.reviewManagerName= $event.empName;
	}
	selectedEmp: any;
	selEmp(){
	...
	listEmployee(){
		...
		}else if(this.flowType=='On Board Profile'){
			this.router.navigateByUrl("/director/on-board-profile")
		}else{
			this.router.navigateByUrl("/operation/employee/list");
		}
	}
	addEmployee() {
	...
	OnBoardEmployee(action){
		...
		if(this.employee.dob == '' || this.employee.dob ==null){
			this.msg = { type: 'danger', text: "Personal Info -> DOB is mandatory" };
			return false;
		}
		if(this.employee.gender == '' || this.employee.gender ==null){
			this.msg = { type: 'danger', text: "Personal Info -> Gender is mandatory" };
			return false;
		}
		this.form.patchValue({
			managerName: this.employee.managerName,
			reviewManagerName: this.employee.reviewManagerName,
		...
		if(this.form.valid){
			...
			this._service.employeeUpdate(this.employee.id,action,this.employee).subscribe(x=>{
				this.prepareDocInfosForCheckin();
				this.CheckIn();
				if(action=='profile-validate' || action=='profile-update-by-hr' || action=='official-info'){
					this.router.navigateByUrl("/operation/employee/list");
			...
			if(this.flowType=="Edit Employee"){
	...
	createEmp(){
		...
		if(this.form.valid){
			...
			this._service.createProfile(this.employee).subscribe(x=>{
				...
				this.prepareDocInfosForCheckin();
				this.CheckIn();
				this.router.navigate(['/operation/employee/detail/'] ,{queryParams:{flowType:"Edit Employee",id:this.employee.id}});
