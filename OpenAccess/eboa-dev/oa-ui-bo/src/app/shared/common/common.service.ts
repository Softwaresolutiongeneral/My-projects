...
for(let i = 2018; i < 2040; i++) {
  ...
  fetchDistrict(){
    if (this.cachedDistrict) {
      return Observable.of(this.cachedDistrict);
    } else {
      // return this.http.get(environment.assetsApiUrl+"/assets/data/substations.json")
      return this.http.get(environment.reportUrl + "report/district-summaries", new CustomRequestOptions())
  ...
  FetchReasonForDeleteTxn()  {
    if (this.cachedDeleteReasons) {
      return Observable.of(this.cachedDeleteReasons);
     } else {
        return this.http.get(environment.reportUrl + "report/delete-transaction-reason-summaries", new CustomRequestOptions())
        .map(res => res.json())
        .do((data) => {
          this.cachedDeleteReasons = data;
        });
    }
  }
  ...
  fetchSubstationsByOrgIdAndTypeOfSs(orgId: string,typeOfSs:string) {

   
      
      return this.http.get(environment.reportUrl + "report/substation-summaries?orgId=" + orgId+"&type-of-ss="+typeOfSs, new CustomRequestOptions()).map((res: Response) => res.json());
  ...
  getMeterAndService(orgId: string, typeOfSs: string) {
    ...
    if (typeOfSs != "") {
      if (typeOfSs != "" && typeOfSs != null) {
        url = url + "&typeOfSs=" + typeOfSs;
      }
      console.log(typeOfSs)
...
export class CustomRequestOptionsforfile extends BaseRequestOptions {
  static map: any;
  constructor() {
    super();
    var token = sessionStorage.getItem('token');
