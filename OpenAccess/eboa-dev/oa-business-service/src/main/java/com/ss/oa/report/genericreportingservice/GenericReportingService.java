...
public class GenericReportingService{
	...
	@Value("${NameTransferprint.url}")
	private String NameTransferprintirl;
	
	
	@Value("${NameTransferprintBill.url}")
	private String NameTransferprintBillurl;
	...
	public PrintPayload printNameChangeDoc(PrintPayload payload) throws IOException {
		System.out.println(payload);		
		RestTemplate restTemplate = getTemplate();

		HttpEntity<PrintPayload> request = new HttpEntity<PrintPayload>(payload, CommonUtils.getHttpHeader("", ""));

		payload  = restTemplate.postForObject(NameTransferprintirl,request, PrintPayload.class); 

		return payload;
	}
	public PrintPayload printNameChangeBill(PrintPayload payload) throws IOException {
		System.out.println(payload);		
		RestTemplate restTemplate = getTemplate();

		HttpEntity<PrintPayload> request = new HttpEntity<PrintPayload>(payload, CommonUtils.getHttpHeader("", ""));

		payload  = restTemplate.postForObject(NameTransferprintBillurl,request, PrintPayload.class); 

		return payload;
	}
	
	public PrintPayload printNameChangePayedBill(PrintPayload payload) throws IOException {
		System.out.println(payload);		
		RestTemplate restTemplate = getTemplate();
