...
public class SQLHandler{
	...
	public static String GetCustomQuery(String reportName, Map<String,String> criteria){
		...
		case "IMPORT-ENERGY-CHARGE REPORT":
			returnSql = getImportEnergycharge();
			break;
		  
		case "Meter-change-reportenterednow":
            returnSql = getMeterChangeEnetered();
            break;
...
private static String getImportEnergycharge() {
	 String sqlQuery ="select * from IEC_REPORT " + 
	 		"where 1=1 ";
	 if(ipCriteria != null && !ipCriteria.isEmpty()){
		if(ipCriteria.containsKey("ip2")){
			sqlQuery += "and MONTH LIKE '%" + ipCriteria.get("ip2") + "%'";
		...
		if(ipCriteria.containsKey("ip1")){
			sqlQuery += "and CIRCLENAME LIKE '%" + ipCriteria.get("ip1") + "%' ";
		}
	}
	return sqlQuery;
}
private static String  getMeterChangeEnetered() {
    String sqlQuery ="SELECT tbl.serviceno,tbl.OLDMETERNO,tbl.newmeterno,tbl.modifydate ,tbl.MODIFIEDDATE,\r\n"
            + "TO_CHAR(tbl.modifydate,'MM'),TO_CHAR(tbl.modifydate,'YYYY'),ser.m_org_name \r\n"
            + "FROM TBL_HIS_MASTER_METER_CHANGEAMR tbl \r\n"
            + "left join v_company_service ser on tbl.serviceno = ser.\"number\" \r\n" +
             "where 1=1"  ;
    if(ipCriteria != null && !ipCriteria.isEmpty()){
       if(ipCriteria.containsKey("ip1")){
           sqlQuery += " and TO_CHAR(tbl.modifydate,'MM') LIKE '%" + ipCriteria.get("ip1") + "%'";
       }
       if(ipCriteria.containsKey("ip2")){
           sqlQuery += " and TO_CHAR(tbl.modifydate,'YYYY') LIKE '%" + ipCriteria.get("ip2") + "%' ";
       }
       if(ipCriteria.containsKey("ip3")){
           sqlQuery += "and ser.m_org_id LIKE '%" + ipCriteria.get("ip3") + "%' ";
       }
      
   }
    sqlQuery +="order by tbl.serviceno";
   return sqlQuery;   
}
