...
import { VerifyContactInfoDialogComponent } from './../verify-contact-info-dialog/verify-contact-info-dialog.component';
import { AuthenticationService } from '../authentication.service';

import * as Ps from 'perfect-scrollbar';
import { VerifyContactInfoNameChangeDialogComponent } from '../../master/name-change-appl/name-change-appl-verify-new-user/name-change-appl-verify-new-user.component';
import { NameChangeApplmodel } from '../../master/vo/nametransfer';
...
export class AdminLayoutComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.getMenus();
        this.openContactVerificationDialog();
    ...
    openContactVerificationDialog(): void {
        //fetch contact-info from company-servic
        //if contactinfo notVerified, then show dialog
        //else do nothing
        let dialogRef = this.dialog.open(VerifyContactInfoDialogComponent, {
            width: '1300px',
            data: {}
        });

        dialogRef.afterClosed().subscribe(result => {
            if (result) this.saveVerificationInfo();
            else this.logout();

        });
    }
    openContactVerificationDialogNameTransfer(applicationdetails:number):void {
        //fetch contact-info from company-servic
        //if contactinfo notVerified, then show dialog
        //else do nothing
        let dialogRef = this.dialog.open(VerifyContactInfoNameChangeDialogComponent, {
            width: '1300px',
