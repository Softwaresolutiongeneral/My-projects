...
export class CommonUtils {
  ...
  public isAlphaNumeric( checkString: string, length: number){ 
    var regex;
    if(length < 1){
      regex = new RegExp("^[a-zA-Z0-9]+$");
    }
    else{
      regex = new RegExp("^[a-zA-Z0-9]{"+length+",}$");
    }
    return regex.test(checkString);
  }
  public isNullOrEmpty(checkString: string){
  ...
  public toFloat( inputString: string){
    inputString = inputString==null?  "0" :  inputString.toString().replace("[0-9]+(\.[0-9]{1,4}?)?", "");
  ...
  getHeaders(){
    ...
    let user : AuthUser = JSON.parse(sessionStorage.getItem('user'));
    headers.append('username', user.userName);
    headers.append('token', user.token);
    headers.append('systemkeycode', user.systemKeyCode);
    return headers;
  }
 ...
 static getLoginUserEDC(){
  ...
  static getIsMaterConfirmed(){
    var user = JSON.parse(sessionStorage.getItem('user'));
  
    if(user.masterConfirmed==null){
      return "";
    }
    else
      {
        // console.log("is masterConfirmed"+user.masterConfirmed);
        return user.masterConfirmed.trim();
      }
  } 
